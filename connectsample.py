...
msgraphapi = oauth.remote_app( \
    ...
    request_token_params={'scope': 'User.Read Mail.Send Mail.Read Mail.ReadWrite Calendars.Read Calendars.ReadWrite'},
...
def authorized():
    ...
    f_token.write(str(session['access_token']))
...
def mytest():
    print('asb')
    get_mail_url = 'https://graph.microsoft.com/v1.0/me/memberOf'
    ...
    response = requests.get(url=get_mail_url,
                             headers=headers,
                             verify=False,
                             params=None)
...
def get_event():
    print('asb')
    get_mail_url = 'https://graph.microsoft.com/v1.0/me/events'
    headers = {'User-Agent' : 'python_tutorial/1.0',
               'Authorization' : 'Bearer {0}'.format(get_token()),
               'Accept' : 'application/json',
               'Content-Type' : 'application/json'}
    ...
    subjectAll = re.findall(patternSubject,result)
#    f = urllib.request.Request(get_mail_url,headers=headers)
#    res = urllib.request.urlopen(f)
#    print('-----------------------------------------------')
#    print(res.read())
...
#    print(res.read())
    ...
    patternSubject = re.compile(r'subject":"(.*?)"')
    patternContent = re.compile(r'content":"(.*?)"}')
    idAll = re.findall(patternID,result)
    subjectAll = re.findall(patternSubject,result)
    contentAll = re.findall(patternContent,result)
    contentAll = re.sub('[\\\ r n]','',str(contentAll))
    if response.ok:
        #response.encoding = 'gbk'
        return str(subjectaAll)
    else:
        return '{0}: {1}'.format(response.status_code, response.text)
...
def get_calendars():
    print('asb')
    get_mail_url = 'https://graph.microsoft.com/v1.0/me/calendars'
    headers = {'User-Agent' : 'python_tutorial/1.0',
               'Authorization' : 'Bearer {0}'.format(get_token()),
               'Accept' : 'application/json',
               'Content-Type' : 'application/json'}
